...
Master v1.4.1 (RULING-028, RULING-029). Conforms to the document standard (v1.4.0). Supersedes in full the
...
- Progress record: the target repo's tightened documents are listed under FF-021 in forgeflow `.forge/backlog.yaml`
  at the forgeflow wrap, one line per document with the merge commit SHA.
...
0. Matt names the repo and the document(s). Claude Web restates them and the lane in one line.
...
6. RECOMMEND in one plain sentence per PR; Matt's yes in chat; squash-merge through the connector; confirm the
   head branch is gone.
7. RECORD at the forgeflow wrap: the document and its merge SHA under FF-021 in `.forge/backlog.yaml`; the target
   repo's inbox file is consumed by that repo's own next wrap, not by forgeflow's.

...
- This repo runs ChromaQA as a pre-push gate (`./chromaqa-check.sh`, also the `quality-gate` job in CI). Where this file carries a ChromaQA section installed by the tool, that section is binding too; a repo without one (the ChromaQA tool repo itself) runs on this line alone. Exit 2 (CAPTAIN REVIEW) means STOP: do not push, do not fix it to make it pass, report the finding as printed with its code and wait — the resolution is Matt's (a code fix, or his acceptance by name, which Claude Web records in `.chromaqa/accepted.json`; no run writes that file). Never `--no-verify`; never delete, empty or edit `.chromaqa/profiles.json` or `.chromaqa/accepted.json`. `.chromaqa/CQA-debt.jsonl` and `CQA-showstoppers.jsonl` showing modified right after a push is the gate working, not a scope breach — the wrap commits them; on ChromaQA 7.2+ a modified debt ledger means a NEW finding, so read the diff and name it.
<!-- <<< end render_when -->
<!-- >>> render_when: subsystems.seogeo -->
- **SEO/GEO:** if `.seogeo/config.json` exists in this repo, read `.seogeo/STANDARD.md` before writing pages (and `.seogeo/CONFIG-REFERENCE.md` before changing a surface). New route groups must be declared in `.seogeo/config.json` in the same session that creates them, or they ship unindexed. Private pages call `noindexMetadataFor` from `@/lib/seo/page-metadata` and that name is never renamed — ChromaQA greps for it. Never move or delete `.seogeo/config.json`.
<!-- <<< end render_when -->
